---
title: "Simulations results: change of phenotypes - filtered 1"
...
plotsFolder = "../plots/mainResults_evolutionOfPhenotypes_filtered_1"
...
sink(paste0(plotsFolder, "/infoMeasures_relDeltaVar-centralityMetrics-neutrality.txt"))
...
# write MI and p values to text file
sink(paste0(plotsFolder, "/infoMeasures_relDeltaVar-centralityMetrics-selection.txt"))
cat(paste0("Variables: relDeltaVar_10000; absInStrT_sqrt\n",
    "Observed MI: ", obs_MI_absInStrT_sqrt, "; pval: ", pval_MI_absInStrT, "\n", 
    "Variables: relDeltaVar_10000; absOutStrT_sqrt\n",
    "Observed MI: ", obs_MI_absOutStrT_sqrt, "; pval: ", pval_MI_absOutStrT, "\n"))
...
```{r stratified-data-lme}
lmeModel <- lme(sqrt(relDeltaVar_10000 + 1) ~ absInStrT_sqrt, 
                data = justRegulated[!is.na(justRegulated$relDeltaVar_10000), ],
                weights = varExp(form = ~absInStrT_sqrt),
                random = ~1|net, na.action=na.exclude,
                method = "ML") 
summary(lmeModel)
r.squaredGLMM(lmeModel)
...
# double check that the p values are zero before renaming them
if(pval_explVar1 == 0){pval_coef1_title = "p < 2.2e-16"} else
  {pval_coef1_title = paste0("p = ", pval_explVar1)}

coef_pval_explVar1_title <- TeX(paste0("$\\beta$ = ", coef_explVar1, "; ", pval_coef1_title))

# R2m for plotting
R2m_absInStr_num <- signif(r.squaredGLMM(lmeModel)[1], 2)
R2m_absInStr_text <- TeX(paste0("R^{2}_m = ", R2m_absInStr_num))

plot_relDeltaVar_absInStr_sel <- ggplot(justRegulated[!is.na(justRegulated$relDeltaVar_10000), ],
                                        aes(x = absInStrT_sqrt, y = sqrt(relDeltaVar_10000 + 1))) +
  geom_hex(bins = 50) +
  geom_abline(slope = lmeModel$coefficients[1]$fixed[2], 
              intercept = lmeModel$coefficients[1]$fixed[1],
              color = "black", linetype = "dashed", size = 1) +
...
plot_relDeltaVar_absInStr_sel




lmeModel <- lme(sqrt(relDeltaVar_10000 + 1) ~ absOutStrT_sqrt, 
                data = justRegulators[!is.na(justRegulators$relDeltaVar_10000), ],
                weights = varExp(form = ~absOutStrT_sqrt),
                random = ~1|net, na.action=na.exclude,
                method = "ML") 
summary(lmeModel)
...
# R2m for plotting
R2m_absOutStr_num <- signif(r.squaredGLMM(lmeModel)[1], 2)
R2m_absOutStr_text <- TeX(paste0("R^{2}_m = ", R2m_absOutStr_num))

plot_relDeltaVar_absOutStr_sel <- ggplot(justRegulators[!is.na(justRegulators$relDeltaVar_10000), ],
                                        aes(x = absOutStrT_sqrt, y = sqrt(relDeltaVar_10000 + 1))) +
  geom_hex(bins = 50) +
  geom_abline(slope = lmeModel$coefficients[1]$fixed[2], 
              intercept = lmeModel$coefficients[1]$fixed[1],
              color = "black", linetype = "dashed", size = 1) +
